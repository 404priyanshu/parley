...
// Must stay in step with APP_IDS in electron-builder.config.ts: this one drives
// the userData directory and the app user model id at runtime.
...
const main = Effect.gen(function* () {
  ...
  const appId = app.isPackaged ? APP_IDS[CHANNEL] : "co.parley.desktop.dev"
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter((arg: string) => arg.startsWith("parley://"))
    if (urls.length) {
  ...
  // Parley registers its own scheme. Claiming "opencode" would hijack the real
  // OpenCode's deep links whenever both apps are installed.
  app.setAsDefaultProtocolClient("parley")
  registerRendererProtocol()
  setDockIcon()
